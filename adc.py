...
class TechniqueResult:
    ...
    def as_navigator(self) -> Iterable[Dict]:
        for tactic in self.tactics:
            if not self.detections:  # Check if detections list is empty
                yield {
                    ...
                    "metadata": [],
                    "enabled": True,
                    "comment": "Darktrace",  # If detections list is empty, set comment to "OK"
                    "showSubtechniques": False,
                    "score": self.score,  # Score can be set to 0 since there are no detections
                }
            else:
        ...
        print(f"TechniqueID: {self.technique_id}, Score: {self.score}")
        #for tactic in self.tactics:
        #    for detection in self.detections:
        #        yield {
        #            "techniqueID": self.technique_id,
